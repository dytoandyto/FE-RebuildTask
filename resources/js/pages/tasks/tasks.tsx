...
import { TaskHeader } from '@/components/tasks/tasks/tasksHeader';
import { TaskStats } from '@/components/tasks/tasks/tasksStats';
...
import { TaskFilters } from '@/components/tasks/tasks/tasksfilter';
...
export default function Tasks() {
    ...
    // --- 2. Filter Logic (Board & Frontend Search) ---
    ...
    const handleDateFilter = (range: DateRange | undefined) => {
        setDateRange(range); // Simpan ke state, React bakal otomatis re-render
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {selectedTask && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setSelectedTask(null)}>
                    <div className="bg-[#0a0a0a] border border-white/10 p-8 rounded-[32px] max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
                        <h2 className="text-white font-bold text-xl italic">{selectedTask.title}</h2>
                        <p className="text-neutral-400 mt-4">{selectedTask.description || "No description available."}</p>
                        <button
                            className="mt-6 w-full py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors"
