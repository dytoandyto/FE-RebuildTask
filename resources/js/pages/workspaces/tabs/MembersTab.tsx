...
import { 
    Search, UserPlus, Mail, MoreVertical, Crown, ShieldCheck, UserCog, User, Users2, Briefcase, Filter, X, LayoutGrid, List } from "lucide-react";
    ...
    const memberColumns = [
            ...
            createdCell: (cell: any, cellData: any, rowData: any) => {
                createRoot(cell).render(
                    <div className="flex items-center gap-3">
                        <div className="size-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary font-bold text-[10px]">
                            {rowData.initials}
                        </div>
                        <div>
                            <p className="font-bold text-sm leading-tight">{rowData.name}</p>
                            <p className="text-[10px] text-muted-foreground leading-tight">{rowData.email}</p>
                        </div>
                    </div>
                );
            }
        },
        {
            ...
            createdCell: (cell: any, cellData: any, rowData: any) => {
                ...
                createRoot(cell).render(
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-widest ${config.class}`}>
            ...
            createdCell: (cell: any, cellData: any, rowData: any) => {
                createRoot(cell).render(
                    <div className="flex items-center gap-2">
                        <div className={`size-1.5 rounded-full ${rowData.status === 'active' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                        <span className="text-[10px] font-bold capitalize">{rowData.status}</span>
                    </div>
                );
            }
        },
